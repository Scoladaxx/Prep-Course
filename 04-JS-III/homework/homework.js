...
function numeroMasGrande(numeros) {
  ...
  for (i = 1 ; i < numeros.length ; i++){
    if (numeros[i] > result) {result = numeros[i]}
  }
  return result;
}
...
function cuentoElementos(arreglo){
  ...
  for (i = 0 ; i < arreglo.length ; i++){
    if (arreglo[i] > 18) {result = result + 1}
    else {null};
  }
  return result;
}
...
function mesesDelAño(array) {
  ...
  var wresult = [];
  for (i = 0 ; i < array.length ; i++){
    if (array[i] === "Enero" || array[i] === "Marzo" || array[i] === "Noviembre") {result.push(array[i])}
    else {wresult[i] = array[i]};
  }
  if (result.length === 3) {
    return result}
...
function breakStatement(numero) {
  ...
  for (i = 0 ; i <10 ; i++){
    numero = numero + 2;
    suma[i] = numero;
  if (suma[i] === i) {return 'Se interrumpió la ejecución'}
  else {null};
  }
  return suma;
}
