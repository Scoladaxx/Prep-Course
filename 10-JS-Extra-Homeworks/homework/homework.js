...
function numberOfCharacters(string) {
  ...
  for (i = 0; i < string.length; i++){
    if (objeto.hasOwnProperty(string[i])) {objeto[string[i]] = objeto[string[i]] + 1}
...
function asAmirror(str) {
  ...
  for (i=0; i < array1.length; i++) {
    for (j=(array1[i].length - 1); j>-1; j--) {
      reverse = reverse + array1[i][j];
    }
    array1[i] = reverse;
    reverse = '';
  }
...
function capicua(numero){
  ...
  for (i=(number.length - 1); i>-1; i--) {
    numreverse = numreverse + number[i];
  }
